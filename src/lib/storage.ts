...
export type ListItem = { id: string; itemId: string; quantity: number; checked: boolean; price: number; unit?: 'unidade' | 'peso'; createdBy?: string }
...
function ensureInit(): void {
  // Migração: se existir dados antigos não escopados para o usuário atual, migra
  ...
  // Inicializar store global compartilhado se não existir
...
export const storage = {
  // Inicializa o escopo do usuário atual e garante seeds
  ...
  async getLists(): Promise<ShoppingList[]> {
    ...
    // Retorna união (sem deduplicar IDs, pois stores são distintos por design)
    return [...shared, ...mine]
  },
  ...
  async updateList(id: string, patch: Partial<ShoppingList>): Promise<ShoppingList> {
    ...
    if (idx !== -1) {
      ...
      // permanece no escopo pessoal
      lists[idx] = updated
      writeJson(K.lists, lists)
      return updated
    }
    // Tenta no store compartilhado
    const shared = readGlobalJson<ShoppingList[]>(K.sharedLists, [])
    const sidx = shared.findIndex(l => l.id === id)
  ...
  async promoteListToShared(id: string): Promise<ShoppingList> {
    ...
    // Se não estiver no pessoal, retorna do compartilhado (ou erro se não existir)
    const shared = readGlobalJson<ShoppingList[]>(K.sharedLists, [])
  ...
  async deleteList(listId: string): Promise<void> {
    // tenta remover do escopo pessoal
    const mine = readJson<ShoppingList[]>(K.lists, [])
    ...
    // se não estiver no pessoal, tenta remover do store compartilhado
    const shared = readGlobalJson<ShoppingList[]>(K.sharedLists, [])
    ...
    // se não encontrar em nenhum, não faz nada
  },
